...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    T = key.size(-2)
    bs = key.size(0)
    S = query @ key.transpose(-2,-1)/torch.sqrt(torch.tensor(dim))

    # Pad then normalize 
    S_pad = F.softmax(S.masked_fill(attention_mask[:,:,:T,:T] != 0, -1e20), dim=-1)

    # Weighted values 
    att_weighted = S_pad @ value
    att_weighted = att_weighted.transpose(1, 2).contiguous().view(bs, T, -1)
    return att_weighted
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    bl = ln_layer(input + dropout(dense_layer(output)))
    return bl
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    ### TODO
    h1 = self.self_attention.forward(hidden_states, attention_mask)
    h2 = self.add_norm(hidden_states, h1, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    h3 = self.interm_af(self.interm_dense(h2))
    h4 = self.add_norm(h2, h3, self.out_dense, self.out_dropout, self.out_layer_norm)
    return h4
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    embd_add = inputs_embeds + pos_embeds + tk_type_embeds
    out = self.embed_dropout(self.embed_layer_norm(embd_add))
    return out
